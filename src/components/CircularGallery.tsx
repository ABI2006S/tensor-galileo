...
class Title {
  gl: any; plane: any; renderer: any; text: string; textColor: string; font: string; mesh: any;
...
class Media {
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    this.scale = this.screen.height / 1200; // Increased base scale (was 1500)
    // Pushed up the horizontal / vertical scale multiplier for larger images
    ...
    this.padding = 2.5; // Increased spacing slightly
...
class App {
  ...
  createRenderer() {
    this.renderer = new Renderer({ alpha: true, antialias: true, dpr: Math.min(typeof window !== 'undefined' ? window.devicePixelRatio : 1, 2) });
  ...
  onResize() {
    ...
    // Pull the camera in slightly to fill the screen more
